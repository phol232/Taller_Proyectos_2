...
import type { Conflict, TimetableSlot } from "@/types/schedule";
...
function componentLabel(type: string): string {
  ...
  return COMPONENT_LABEL[key] ?? type;
}
...
export default function WeeklyGrid({
  ...
  conflictCourseIds,
  conflicts = [],
  emptyMessage = "No hay bloques para mostrar.",
}: WeeklyGridProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className="grid divide-x divide-border overflow-x-auto rounded-xl ring-1 ring-border"
        style={{ gridTemplateColumns: `repeat(${Math.max(activeDays.length, 1)}, minmax(140px, 1fr))` }}
      >
        {activeDays.map((day) => {
            ...
            <div key={day.key} className="min-w-0">
              ...
              <div className="space-y-1.5 p-2">
                {daySlots.map((slot) => {
                  ...
                  const hasConflict = conflictCourseIds?.has(slot.courseId);
                  return (
                    <div
                      key={slot.slotId}
                      role={onCourseClick ? "button" : undefined}
                      tabIndex={onCourseClick ? 0 : undefined}
                      onClick={onCourseClick ? () => onCourseClick(slot.courseId) : undefined}
                      onKeyDown={
                        onCourseClick
                          ? (e) => {
                              if (e.key === "Enter" || e.key === " ") {
                                e.preventDefault();
                                onCourseClick(slot.courseId);
                              }
                            }
                          : undefined
                      }
                      className={cn(
                        "relative rounded-lg border px-2 py-2 text-xs leading-tight",
                        c.bg,
                        hasConflict ? "border-ship-red ring-1 ring-ship-red/40" : c.border,
                        c.text,
                        onCourseClick && "cursor-pointer transition-shadow hover:shadow-md hover:ring-1 hover:ring-border",
                      )}
                    >
                      ...
                      {slot.nrc && (
                        <p className="truncate text-[10px] opacity-55">NRC {slot.nrc}</p>
                      )}
